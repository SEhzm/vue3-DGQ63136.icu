...
const startDrag = (event, component) => {
    ...
    // 记录初始元素位置
    const initialX = component === 'chat' ? chatX.value : audioX.value;
    const initialY = component === 'chat' ? chatY.value : audioY.value;
    ...
    const dragging = (moveEvent) => {
        ...
        if (isDragging.value) {
            ...
            if (component === 'chat') {
                ...
                audioX.value = initialX + deltaX / 19.2;
                audioY.value = initialY + deltaY;
            }
        }
    };
