            ...
            <div class="barrage-main-content">
                ...
                <div class="barrage-meta-info">
                    ...
                    <div class="submit-time">
                        ...
                        {{ formatDateTime(randomMeme.submitTime) }}
                    </div>
                </div>
            </div>
...
function getDisplayTags(tagsStr: string, tagList: any[]) {
    if (!tagsStr) return [];
    const list = Array.from(new Set(tagsStr.split(',').map((t) => t.trim())));
    return list.map((v) => {
        const dictItem = tagList.find((it) => String(it.dictValue).trim() === v);
        return dictItem
            ? { dictValue: v, label: dictItem.dictLabel, iconUrl: dictItem.iconUrl }
            : { dictValue: v, label: v, iconUrl: '' };
    });
}
