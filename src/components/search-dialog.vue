...
const memeTags = computed(() => memeTagsStore.tags);
...
async function performSearch(searchKey: string, sortType: SortType = SortType.ID, pageNum: number = 1, tags?: string[], timeRange?: [string, string]) {
    ...
    // 后端 sort 是 0=按时间，1=按复制次数
    const sortValue = sortType === SortType.COPY ? 1 : 0;

    let res: any;
    try {
        res = await httpInstance.post(API.SEARCH_MEME_ADV, {
            barrage: searchKey,
            sort: sortValue,
            pageNum,
            tags: tags?.join(','),
