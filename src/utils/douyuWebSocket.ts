...
function WebSocket_Packet(str: string): Uint8Array {
    ...
    const bufferSize = bytesArr.length + 4 + 4 + 2 + 1 + 1 + 1;
    const buffer = new Uint8Array(bufferSize);
    const p_content = new Uint8Array(bytesArr.length);

    for (let i = 0; i < p_content.length; i++) {
        p_content[i] = bytesArr[i];
    }

    const p_length = new Uint32Array([bytesArr.length + 4 + 2 + 1 + 1 + 1]);
    const p_type = new Uint32Array([MSG_TYPE]);
...
export function handleDanmu(msg: string): void {
    ...
    if (oni) {
        ...
        // 兼容两种写法
        if (typeof (store as any).updateOni === 'function') {
            (store as any).updateOni(oni);
        } else if (typeof (store as any).setOni === 'function') {
            (store as any).setOni(Number(oni));
        }
    }
...
function showNotification() {
    const notification = new Notification('来自 dgq63136.cn 开播提醒', {
        body: '63136 冬瓜强直播间已开播，快去观看吧！',
    });

    notification.onclick = () => {
        window.focus();
        window.open('https://www.douyu.com/63136', '_blank');
    };
}
