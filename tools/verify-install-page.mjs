...
assert.ok(page.includes("currentPluginVersion = 'V0.2.28'"), "install page should show current display version");
assert.ok(page.includes("currentPluginUpdatedAt = '2026-08-18 05:28'"), "install page should show current update time");
assert.ok(page.includes("/dgq63136.user.js?v=202608180528"), "install button should use cache-busted packaged userscript path");
...
assert.ok(page.includes("version: 'V0.2.28'") && page.includes("version: 'V0.2.17'"), "history should include recent plugin versions");
...
assert.ok(publicUserscript.includes("// @version      2026.08.18.02"), "public userscript should use current technical version");
assert.ok(publicUserscript.includes('DISPLAY_VERSION = "V0.2.28"'), "public userscript should use current display version");
...
assert.ok(publicUserscript.includes('HOTWALL_SETTLE_MS = 1200'), 'hotwall should settle quickly after receiving data');
assert.ok(publicUserscript.includes('相同或相似内容'), 'public userscript should use clearer duplicate warning');